...
Do NOT modify: src/App.tsx、renderMarkdown.ts、檔案整合（fileOps）、主題/設定、既有測試。
T09 將在所有 feature modules 合併後統一把 HTML export seam 接到 App toolbar；本票不要自行修改共享 UI。
...
2. 匯出支援 seam：提供可由 App 呼叫的純/非 UI API，並保留 Rust save dialog → 寫檔 → 成功/取消/寫入失敗三路徑的明確結果；T09 負責接線與提示，不在本票修改 App。
